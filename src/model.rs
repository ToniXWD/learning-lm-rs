...
fn mlp(
    ...
    operators::rms_norm(hidden_states, &residual, rms_w, eps);
    operators::matmul_transb(gate, 0., &hidden_states, &w_gate, 1.);
    operators::matmul_transb(up, 0., &hidden_states, &w_up, 1.);
    operators::swiglu(up, gate); // 这里的输出存在 up 中, 就是文档的 act
    operators::matmul_transb(residual, 1., &up, &w_down, 1.);
}
